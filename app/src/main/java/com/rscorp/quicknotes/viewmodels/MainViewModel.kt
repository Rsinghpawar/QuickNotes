...
class MainViewModel @ViewModelInject constructor(private val notesDao: NotesDao, private val currentNotesDao : CurrentNotesDao) : ViewModel() {
    ...
    fun updateTable(id : Int ,title : String , icon : Int , iconPosition : Int , tag : String ) = viewModelScope.launch(Dispatchers.IO) {
            currentNotesDao.updateNote(id, title, icon, iconPosition , tag)
    }
